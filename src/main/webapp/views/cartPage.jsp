...
<head>
	...
	<script>
		function updateProduct(productId) {
			...
			window.location.href = `<c:url value='/updateCart'/>?productId=${productId}&quantity=${updatedQuantity}`;
		}
	</script>
</head>
<body class="bg-light">
				...
				<div class="col-md-3">
					<div class="card mb-4">
						...
						<div class="card-body">
							...
							Quantity:
							<input type="number" min="1" max="${product.quantity}" value="1" onkeydown="return false;" id="quantity-${product.id}">
							<button class="btn btn-primary" onclick="updateProduct(${product.id})">Update</button>

							<a href="<c:url value='/deleteCartProduct'/>?productId=${product.id}"
							   class="btn btn-primary btn-primary-danger" id="addToCartBtn${product.id}">delete</a>
